...
## 2. Для клонирования этого проекта на виртуальную машину (*тут будет более короткая инструкция*).

## Installation 

* Подключение по ssh к виртуальной машине (в PowerShell в корне своего пользователя):

```sh
ssh user23@192.144.14.9
```
* Запуск оболочки bash (чтобы было приятней работать в PowerShell):

```sh
bash
```
* Обновление индексов пакетов системы:

```sh
sudo apt update
```
* Установка модуля venv для python3:

```sh
sudo apt install python3-venv
```
* Клонирование своего репозитория, создание виртуального окружения, активация и установка зависимостей:
...
pip install -r requirements.txt
```
* Копирование файла `.env` из локальный машины на сервер:
Открыть второе окно PowerShell в директории, где есть файл `.env`:
...
### 1. Загрузка данных из S3 на виртуальную машину  
...
Для использования сервиса используйте файл `web/index.html`. \
**Проверьте, чтобы в `endpoint` стоял ваш публичный IP**.

### Результаты лабораторных работ (dev-prod)
1) Адрес, на котором запущен Ваш сервис предсказания цен: '[http://192.144.14.9:8000/predict](http://192.144.14.9:8000/predict)'.
...
```sh
docker run -d -p 8000:8000 klantsova/pabd24:latest
```